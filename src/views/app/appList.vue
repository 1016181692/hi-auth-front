<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="userTable.queryParams.name" placeholder="应用简称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
      <el-input v-model="userTable.queryParams.username" placeholder="应用名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
    ...
    <el-table
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="app-table-expand">
            ...
            <el-form-item label="回调地址">
              <span>{{ props.row.webServerRedirectUris }}</span>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
      <el-table-column label="图标" width="100" align="center">
        <template slot-scope="scope">
          <img :src="scope.row.img1X" style="width: 50px;height: 50px">
        </template>
      </el-table-column>
...
import { queryApp, savaApp } from '@/api/app'
...
export default {
  ...
  methods: {
    getUserPage() {
      ...
      queryApp(this.userTable.queryParams).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          savaApp(undefined, userData).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const userData = Object.assign({}, this.user)
          savaApp(this.user.id, userData).then(response => {
